...
> Note: You must install a library-package before you load a library-package: `install.packages("tidyverse")`. You only need to install a package once. You must load the package with each R session. The above code chunk engages the `library()` function to load the "*tidyverse*" packages. ([Video Help: Install Pages](https://tutorials.shinyapps.io/00-setup/#section-install-packages))
